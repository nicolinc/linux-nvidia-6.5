...
static void arm_smmu_domain_nested_free(struct iommu_domain *domain)
{
	...
	if (!WARN_ON(!id_user)) {
		struct arm_smmu_domain *s2_parent = nested_domain->s2_parent;
		struct arm_smmu_stream *stream;

		xa_lock(&s2_parent->streams_user);
		stream = __xa_erase(&s2_parent->streams_user, id_user);
		/*
		...
		if (unlikely(stream->id_user != id_user)) {
			WARN_ON(__xa_alloc(&s2_parent->streams_user, &id_user,
					   stream, XA_LIMIT(id_user, id_user),
					   GFP_KERNEL_ACCOUNT));
		} else
			stream->id_user = 0;
		xa_unlock(&s2_parent->streams_user);
	}
				 ...
				 u64 *cmd, u32 *cerror_idx)
{
	...
	case CMDQ_OP_CFGI_CD:
	case CMDQ_OP_CFGI_CD_ALL:
		xa_lock(&nested_domain->s2_parent->streams_user);
		stream = xa_load(&nested_domain->s2_parent->streams_user,
				 FIELD_GET(CMDQ_CFGI_0_SID, *cmd));
		xa_unlock(&nested_domain->s2_parent->streams_user);
		if (!stream) {
			      ...
			      const struct iommu_user_data *user_data)
{
	...
	ret = xa_alloc(&smmu_parent->streams_user, &arg.sid,
			   ...
			   const struct iommu_user_data *user_data)
{
	...
	if (paging_flags & IOMMU_HWPT_ALLOC_NEST_PARENT) {
		...
		xa_init_flags(&smmu_domain->streams_user,
			      XA_FLAGS_ALLOC1 | XA_FLAGS_ACCOUNT);
		smmu_domain->stage = ARM_SMMU_DOMAIN_S2;
	}
